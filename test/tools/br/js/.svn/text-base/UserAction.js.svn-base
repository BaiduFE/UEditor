...
//    flag : true,
    ...
    checkResult : function( range, sc, ec, so, eo, collapsed, descript ) {
        ...
        same( range.startContainer, sc, "check startContainer--" + descript );
        same( range.endContainer, ec, "check endContainer--" + descript );
        equal( range.startOffset, so, "check startOffset--" + descript );
        equal( range.endOffset, eo, "check endOffset--" + descript );
    },
    manualDeleteFillData : function () {
        var domUtils = baidu.editor.dom.domUtils;
        var fillData = baidu.editor.fillData;
        fillData && domUtils.remove( fillData );
        baidu.editor.fillData = null;
    },
...
//                    name = this.cssStyleToDomStyle( name );
    ...
    hasSameAttrs : function( nodeA, nodeB ) {
        ...
        if ( !this.browser.ie && thisAttribs.length != otherAttribs.length )
            return false;
        ...
        if ( k ) while ( k -- ) {
            ...
            if ( !this.browser.ie || thisAttr.specified ) {
                specLen ++;
...
//                    if ( nodeB.getAttribute( thisAttr.nodeName ) != thisAttr.nodeValue ) {
    ...
    checkAllChildAttribs : function( nodeA, nodeB, flag ) {
    ...
    haveSameAllChildAttribs : function( nodeA, nodeB ) {
        var flag = true;
        return this.checkAllChildAttribs( nodeA, nodeB, flag );
    },
